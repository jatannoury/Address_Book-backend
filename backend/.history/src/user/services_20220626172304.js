...
async function Update(type, content, id) {
  console.log(type);
  if (type=="Name")return await Contact.findByIdAndUpdate(id, { fullName: content });
  if (type=="Email")return await Contact.findByIdAndUpdate(id, { email: content });
  if (type=="Phone Number")return await Contact.findByIdAndUpdate(id, { phoneNb: content });
  if(type=="Marital Status") return await Contact.findByIdAndUpdate(id, {rlsStatus : content });
}
module.exports = {
  getByEmail,
  addUser,
