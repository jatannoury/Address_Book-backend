...
async function getContactByEmail(email) {
  ...
  return await Contact.find({ email: email }).populate("user");
}
...
async function Update(type, content, id) {
  console.log(type);
  if (type == "Name")
    return await Contact.findByIdAndUpdate(id, { fullName: content });
  if (type == "Email")
    return await Contact.findByIdAndUpdate(id, { email: content });
  if (type == "Phone Number")
    return await Contact.findByIdAndUpdate(id, { phoneNb: content });
  if (type == "Marital Status")
    return await Contact.findByIdAndUpdate(id, { rlsStatus: content });
}
async function Remove(contact) {
  console.log("YOW YOW", contact);
  return Contact.remove(contact[0]);
}
module.exports = {
  getByEmail,
  addUser,
